      ...
      if ((new File("./cesaves/save0.dat")).exists()) {
        ...
        try {
          ...
          if (flags.get(6) == 1) {
            ...
            rooms[0][0].setDesc("A small, square room with a pedestal in the centre.\n\tA basin of water sits upon it.");
          }
          if (flags.get(7) == 1) {
  ...
  public static void useItem() {
    ...
    if (player.getInventorySize() > 0) {
      ...
      if (itemChoice > 0 && itemChoice <= player.getInventorySize()) {
        if(Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Rusted key") && player.getXcoord() == 1 && player.getYcoord() == 0 && flags.get(0) == 0) {
          flags.set(0, 1);
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Steel key") && player.getXcoord() == 2 && player.getYcoord() == 3 && flags.get(1) == 0) {
          flags.set(1, 1);
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Bucket") && player.getXcoord() == 0 && player.getYcoord() == 0) {
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Bucket of water") && player.getXcoord() == 1 && player.getYcoord() == 2 && flags.get(5) == 0) {
          flags.set(5, 1);
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Pickaxe") && player.getXcoord() == 2 && player.getYcoord() == 4 && flags.get(2) == 0) {
          flags.set(2, 1);
          rooms[2][4].setDesc("An old kitchen with antique counter-tops and pots and pans lining the walls.\n\tThe western wall of the room has been knocked down and rubble clutters the floor...");
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Unlit torch") && player.getXcoord() == 3 && player.getYcoord() == 2 && flags.get(4) == 0) {
          flags.set(4, 1);
        ...
        } else if (Objects.equals(player.getItemFromInventory(itemChoice - 1).getName(), "Golden key") && player.getXcoord() == 1 && player.getYcoord() == 4 && flags.get(3) == 0) {
          flags.set(3, 1);
          rooms[1][4].setDesc("This kitchen was decommissioned long before the other.\n\tIt was sealed off by the wall that stood to the east, which is now a pile of rubble.\n\tThere is hardly anything left of what this kitchen once was.\n\tThere are broken jars in the doorway to the west.");
    ...
    while (itemChoice > 0) {
      ...
      if (currentRoom.getLootSize() > 0) {
        ...
        if (itemChoice > 0 && itemChoice <= currentRoom.getLootSize()) {
          ...
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Steel key") && !Objects.equals(rooms[0][0].getDesc(), "A small, square room with a pedastal in the centre.\n\tA basin of water sits upon it.")) {
            flags.set(6, 1);
            rooms[0][0].setDesc("A small, square room with a pedestal in the centre.\n\tA basin of water sits upon it.");
          }
          //If the Golden key is being picked up, the desc for the Wash Room must change
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Golden key") && !Objects.equals(rooms[2][2].getDesc(), "There is an antique bathtub against the wall and small steps leading to a broken sink.\n\tAn old pipe is exposed which was once underground and none of the plumbing works.\n\tA passage to the south leads to the Old Sewers.")) {
            flags.set(7, 1);
            rooms[2][2].setDesc("There is an antique bathtub against the wall and small steps leading to a broken sink.\n\tAn old pipe is exposed which was once underground and none of the plumbing works.\n\tA passage to the south leads to the Old Sewers.");
          }
          //If the Pickaxe is being picked up, the desc for the Unfinished Excavation must change
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Pickaxe") && !Objects.equals(rooms[3][4].getDesc(), "This excavation was abandoned mid-creation and it's creator lies against the wall to the west.\n\tThe ceiling is supported by rotting wooden shafts and looks as though it could collapse at any moment.\n\tWhat did he discover that led him to such a fate?")) {
            flags.set(8, 1);
            rooms[3][4].setDesc("This excavation was abandoned mid-creation and it's creator lies against the wall to the west.\n\tThe ceiling is supported by rotting wooden shafts and looks as though it could collapse at any moment.\n\tWhat did he discover that led him to such a fate?");
          }
          //If the Bucket is being picked up, the desc for the Tomb Entrance must change
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Bucket") && !Objects.equals(rooms[1][1].getDesc(), "Two large, ornate doors stand wide-open to the south... the air grows colder as you approach them.\n\tIn the north-eastern corner of the room there are some skeletons.")) {
            flags.set(9, 1);
            rooms[1][1].setDesc("Two large, ornate doors stand wide-open to the south... the air grows colder as you approach them.\n\tIn the north-eastern corner of the room there are some skeletons.");
          }
          //If the Unlit torch is being picked up, the desc for the Scullery must change
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Unlit torch") && !Objects.equals(rooms[0][3].getDesc(), "Broken plates and goblets lay piled up on the counters and in cabinets on the walls.\n\tThere is a rather large dried-blood stain on the ground next to the eastern wall.\n\tThis appears to be a dead end.")) {
            flags.set(10, 1);
            rooms[0][3].setDesc("Broken plates and goblets lay piled up on the counters and in cabinets on the walls.\n\tThere is a rather large dried-blood stain on the ground next to the eastern wall.\n\tThis appears to be a dead end.");
          }
          if (Objects.equals(loot.get(itemChoice - 1).getName(), "Treasures")) {
            Helper.tab(1);
